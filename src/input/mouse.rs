...
//! sequence of `UserCommand`s for the main loop to dispatch.
//!
//! *Adapter* (not a router): wraps a device API
//! (`crossterm::MouseEvent`) and produces a different shape
//! (`UserCommand`), in the GoF sense. It does not decide where the
...
//! routes through the [`compositor`](crate::core::compositor) for focus
//! delivery before producing a `UserCommand`, while mouse is a pure
//! translator with no focus involvement. Both emit the same `UserCommand`
...
impl MouseAdapter {
    /// Translate a raw mouse event into zero or more `UserCommand`s.
    ...
    pub fn translate(&mut self, event: MouseEvent) -> Vec<UserCommand> {
        let mut msgs = vec![UserCommand::CursorMoved(event.column, event.row)];
    ...
    fn resolve(&mut self, event: MouseEvent) -> Option<UserCommand> {
        ...
        match event.kind {
            ...
            MouseEventKind::Drag(MouseButton::Left) => {
                if let Some((prev_x, prev_y)) = self.drag_from {
                    ...
                    self.drag_from = Some((event.column, event.row));
                    if dx != 0 || dy != 0 {
                        return Some(UserCommand::Map(MapAction::PanCells(dx, dy)));
                    }
                }
                None
            }
