...
//! `AppMsg`s (and a cursor-tracking side effect), then hands them
//! back to the main loop for `app_msg::dispatch`.
//!
...
//! other Rust TUI apps (gitui). But they now share the same `AppMsg`
//! vocabulary on the output side: drag → `AppMsg::Map(PanCells)`,
//! scroll → `AppMsg::Map(ZoomAt { ... })`.
...
impl MouseHandler {
    ...
    /// `AppMsg` for the main loop to dispatch. `None` means "handled
    /// locally (cursor move, click tracking, modal gate) — no state
    /// change for the dispatcher".
    pub fn handle(&mut self, event: MouseEvent, ui: &mut UiState) -> Option<AppMsg> {
