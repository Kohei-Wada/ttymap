...
impl BrailleBuffer {
    ...
    pub fn set_pixel_punching(&mut self, x: usize, y: usize, color: u8) {
        ...
        if self.pixel_buf[idx] == 0xFF {
            // Saturated cell: keep the fill colour as bg (so OFF dots
            // render against the underlying water/forest/… instead of
            // the global bg) but darken both bg and fg to match the
            // perceived brightness of surrounding `⣿` cells. The font
            // renders Braille dots covering ~30-50% of the cell area,
            // so a `⣿` cell appears at roughly half the bg colour's
            // intensity. A line cell with bg = full water-blue would
            // outshine those neighbours; halving the colour value
            // (`dim_xterm`) brings them to comparable apparent
            // brightness. `Modifier::DIM` is the wrong tool here — it's
            // a foreground-only attribute in every terminal, so the
            // dominant bg-fill would stay bright.
            self.bg_buf[idx] = crate::theme::palette::dim_xterm(self.fg_buf[idx]);
            self.fg_buf[idx] = crate::theme::palette::dim_xterm(color);
        } else {
            // Sparse: replace mask, set fg to overlay (full brightness),
            // leave bg alone. Sparse cells appear mostly dark already
            // (few dots), so the overlay's full-brightness fg matches
            // the visual character of their neighbours.
            self.fg_buf[idx] = color;
        }
        self.pixel_buf[idx] = bit;
    }
...
mod tests {
    ...
    /// - bg = dim_xterm(prior fg) — fill colour darkened to match `⣿` perceived brightness
    /// - fg = dim_xterm(overlay colour) — overlay dot also darkened so it doesn't pop
    #[test]
    fn set_pixel_punching_dims_bg_and_fg_on_saturated_cell() {
        use crate::theme::palette::dim_xterm;
        ...
        assert_eq!(buf.bg_buf[idx], dim_xterm(5), "bg = dim_xterm(prior fg)");
        assert_eq!(
            buf.fg_buf[idx],
            dim_xterm(7),
            "fg = dim_xterm(overlay colour)"
        );
    }
