...
//! gets its `register_palette_command` / `register_keybind` calls
//! captured into a [`CapturedRegistration`]. This module's
//! [`build_subsystem`](crate::lua::build_subsystem) consumes those
//! captures and pushes the result into the fields of a single shared
//! [`Registrar`]:
//!
...
//!
//! Event-bus subscriptions (`ttymap.on_event` / `on_tick`) bypass
//! the registrar — they subscribe directly against the
//! [`EventBus`](crate::event::EventBus) at call time and return a
//! Lua-facing `EventHandle`. The bus itself is built once by
//! `build_subsystem` and shared between the API surface and
//! [`crate::lua::LuaHandle`].
//!
