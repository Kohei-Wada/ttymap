...
17 total. Each plugin is a reference implementation of one shape —
...
provider (`search`), or full game with focused-card input
(`snake`):
...
search/          snake.lua        terminator.lua     travel/
wiki/
```
