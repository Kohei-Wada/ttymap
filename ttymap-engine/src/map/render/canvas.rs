...
impl Canvas {
    ...
    /// [`BrailleBuffer::set_pixel_punching`] at the leaf. Saturated
    /// cells (fully-filled tile fills) have their mask replaced with
    /// just the overlay's single bit, producing a one-cell gap in the
    /// fill. Sparse cells receive the normal OR-merge.
