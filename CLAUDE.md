...
- `ttymap-app/` — composition root + binary. Owns App state hub +
  ...
  pipe), XDG logging. Depends on every other crate. The produced executable is still `ttymap` (set via
...
```
ttymap-engine/                (ratatui-free; produces MapFrame)
  src/
    config.rs                 cache / map / render settings
    geo.rs                    Web Mercator projection math
    ipc.rs                    EngineCommand / EngineEvent + postcard codec
...
ttymap-tui/                   (UI primitives; depends on engine + shared + config)
  src/
    ...
    frame_widget.rs           ratatui Widget newtype for engine MapFrame
                              (orphan-rule wrapper)
    input/                    KeyMap table + mouse adapter + input thread
    overlay.rs                per-frame overlay sink + redraw throttle
                              (drained into RenderTask::Draw)
    palette/                  `:`-triggered picker UI
    sidebar.rs                SidebarPolicy: visibility + width + auto-open
                              on first card / auto-close on last
    theme/                    ratatui adapter — UiTheme + StyleKind
                              (re-exports ColorPalette/ThemeId/DARK/BRIGHT from engine)

ttymap-lua/                   (Lua plugin runtime; mlua + Lua 5.4 vendored)
  src/
    api/                      Rust→Lua API binding (http/json/sgp4/map/config/
                              help/log/tile/storage + register/imperative)
    ...
    tick.rs                   per-frame `tick` registry (separate from EventBus)

ttymap-cli/                   (CLI subcommands; reused by ttymap-app)
  src/
    lib.rs                    clap entry — Command::{Snap, EngineWorker}
    snap.rs                   headless single-frame render to ANSI

ttymap-app/                   (composition root + binary; ratatui + crossterm shell)
  src/
    main.rs                   entry — early-dispatches `engine-worker` subcommand
    app/                      App state hub + event loop
      mod.rs / frame_timer.rs / ui.rs
    engine_handle.rs          parent end of the IPC pipe (pure transport; App owns the MapState)
...
stdin/stdout with a postcard-framed `EngineCommand` / `EngineEvent`
protocol (`ttymap-engine/src/ipc.rs`). The parent owns the only
`MapState` (the camera): on a `MapAction` it updates its `MapState`
and ships the resulting `Viewport` to the engine inside
`EngineCommand::Draw { viewport, overlays }`. The engine holds no
camera state — it renders exactly the viewport it is handed. Lua's
synchronous getters (`ttymap.map:center()`) read the parent's
`MapState` directly, never round-tripping IPC. The engine still owns
the heavy async data state (tile cache + fetch/decode pipeline).
See #348 for
...
- **`ttymap-tui/src/theme/`**: TUI-side ratatui adapter — `ui.rs` (`UiTheme`) and `style.rs` (`StyleKind` semantic tags). `mod.rs` re-exports `ColorPalette` / `ThemeId` / `DARK` / `BRIGHT` from `ttymap_engine::theme` so the rest of the TUI keeps using `crate::theme::*` without caring that the data half lives in the engine crate.
...
- **`ttymap-engine/src/ipc.rs`**: IPC scaffolding for the `ttymap engine-worker` subprocess role. Defines `EngineCommand` (parent → child: Init / Resize / SetTheme / SetLabelsVisible / SetLayerVisible / Draw { viewport, overlays } / Shutdown), `EngineEvent` (child → parent: Ready { attribution } / FrameReady / Error), and the postcard codec (u32-LE length prefix + payload, capped at 16 MB). `run_as_subprocess` is the child entry point — reached via `ttymap engine-worker` (clap `cli::Command::EngineWorker`), early-dispatched in `ttymap-app/src/main.rs` before the Lua runtime path is resolved. Three threads inside the child: command loop on stdin, single-writer fan-in mpsc → stdout, render thread (same as in-process).
- **`ttymap-app/src/engine_handle.rs`**: Parent end of the IPC pipe. `EngineHandle::spawn` does `Command::new(current_exe).arg("engine-worker")`, runs the Init → Ready handshake (synchronously, so spawn returns with `attribution` populated), then spins up `engine-writer` (drains an `mpsc::Sender<EngineCommand>` → child stdin) and `engine-reader` (child stdout → `AppEvent::FrameReady` etc.). Pure transport — it holds **no** `MapState`. The App owns the sole `MapState` (the camera); on a `MapAction` the App mutates it and ships the resulting `Viewport` inside `EngineCommand::Draw`, so Lua's same-tick getters read the camera without blocking and the engine keeps no camera state of its own. `Drop` is the cooperative shutdown: send `Shutdown`, drop the command tx (writer exits → child stdin EOF → child exits), join the two threads, reap the child with a 1 s deadline.
...
- **`ttymap-engine/src/shared/http/`**: User-Agent-tagged `reqwest` wrapper. The engine's tile fetcher (`map/tile/fetch/http.rs`) consumes it directly; the Lua `ttymap.http` bridge (`ttymap-lua/src/api/http.rs`) re-borrows it from `ttymap_engine::shared::http` so there's a single source of truth.
