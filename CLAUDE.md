...
3. **Tile fetch + decode pipeline** (`src/map/tile/`): Three-layer flow `FetchLane → decoder → TileCache`. The fetch lane runs a fixed worker pool over a priority queue (`fetch/lane.rs`) and delegates per-tile bytes acquisition to a `TileFetcher` impl (today only `HttpFetcher`, which also owns the disk cache). A dedicated decoder thread (`decoder.rs`) parses MVT bytes off the render thread and forwards `DecodedTile`s to the cache via `mpsc`.
...
- **`src/map/tile/cache.rs`**: Orchestrator — LRU memory cache (`lru` crate), view state (center / zoom), prefetch ring, and the channel drain (`poll_completed`). No longer touches disk or runs `decode()` after the three-layer-pipe refactor.
- **`src/map/tile/fetch/`**: `TileFetcher` (per-backend trait — "key → bytes") and the generic `FetchLane<F>` (queue / workers / dedup / priority). `http.rs` is the only backend today and also owns the on-disk cache via `directories`. New backends (mbtiles, pmtiles, …) add a `TileFetcher` impl + a branch in `app::build_tile_cache`.
