...
- **`src/app/`**: `App` is the sole Receiver and owns the latest `MapFrame` directly (no `UiState` wrapper — built-in chrome lives in plugins now). `src/app/intent.rs` holds the `UserIntent` intent vocabulary (map-level actions nest under `UserIntent::Map(MapAction)` because `MapState` owns its own vocabulary; other variants sit at the top level); `src/app/event.rs` holds `AppEvent`, the unified queue payload (`Intent` / `FrameReady` / `Input` / `Wake`); `src/app/mod.rs` holds `App::dispatch` (the thin router) and cross-cutting methods like `switch_theme` / `handle_resize`. UI infrastructure lives under `app/`: `compositor/` (focus/modal stack), `palette/` (`:`-triggered picker), `ui.rs` (ratatui draw entry), `frame_timer.rs` (per-iteration wake source). Single side-effect boundary for user-intent state changes. See [docs/design.md](docs/design.md) for the UserIntent-vs-direct-API judgment rules.
- **`src/compositor/`**: Stack-based focus/modal system (helix-inspired). One primitive: a stack of `Component`s, where the top owns key focus. Components render side panels through `Component::render` and can emit `UserIntent` via `Window`. World-space overlays (markers etc.) are *not* a Component concern — every Lua plugin's per-frame map paint runs through the unified `LuaTickRegistry::tick` (called from `ui::draw`) which hands the plugin a `MapApi` it draws into directly. Submodule `map_api.rs` (the drawing facade) is a crate-internal type consumed by the Lua bridge. The earlier `src/plugin_api/` directory that hosted these helpers was a misnamed bucket from the pre-Lua era; it's gone now that every plugin author goes through Lua. `Placement` has two variants: `Floating` (palette-only, drawn over the map) and `Sidebar` (left rail, equal-vertical-split among up to 3 visible cards). Lua plugins always land in `Sidebar`.
