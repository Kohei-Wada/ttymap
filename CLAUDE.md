...
- **`src/plugin/`**: Plugins (attribution, export, help, here, info, iss, quake, scalebar, search, wiki) — composable UI panels with async work via `poll()`. Built-in chrome (info / scalebar / attribution) is structured as plugins too. Search is the unusual one: instead of being a `Component`, it's a `PaletteProvider` (in `src/plugin/search/mod.rs`) — its `register()` binds `/` to push the palette pre-loaded with the provider. Aircraft (live ADS-B) is a Lua plugin and lives under `src/lua/scripts/aircraft.lua`; the same `[aircraft]` config gate routes there now.
- **`src/lua/`**: Lua scripted plugins (mlua + Lua 5.4 vendored). `LuaComponent` (`component.rs`) implements `Component` by dispatching to a Lua module table. `host` (`host.rs`) is a per-component global exposing `host:fetch_url(url)`, `host:jump(lon, lat)`, `host:parse_json(s)`. `MapApi` is bridged via a per-frame Lua table built inside `Lua::scope` (`map_api.rs`). **User plugins**: drop a `*.lua` file into `~/.config/ttymap/plugins/` and `register_user_plugins` picks it up at startup. Each script controls its own activation via the optional `module.enabled` field (default true) — the file *is* the config, no TOML side. `package.path` is extended at Lua-state construction so a plugin can `require "siblings"` from the same directory. Bundled `aircraft.lua` is loaded through `register_aircraft` under the existing `[aircraft]` TOML gate (because the user can't edit a script inside the binary); `register_hello` stays behind `[lua] enabled = true`. See `docs/lua-bridge-surface.md` for the bridge surface scope.
