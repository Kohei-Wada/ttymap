...
- **When to emit a `AppMsg` vs a direct method call** — user intent goes through `app_msg::dispatch`; internal data flow (frame arrival, widget polling) does not.
- **Controller split: by feature, not by domain** — if `app_msg.rs` grows large.
...
1. **Main thread** (`src/app.rs`): Runs the event loop — drains completed frames from the render thread, polls plugins for async work, processes keyboard/mouse/resize events via crossterm, and asks ratatui to paint. State changes driven by user intent flow through `app_msg::dispatch` (`src/app_msg.rs`).
...
- **`src/app_msg.rs`**: App-level command vocabulary and central `dispatch` router. Single side-effect boundary for user-intent state changes. See [docs/design.md](docs/design.md) for the Command-vs-direct-API judgment rules.
