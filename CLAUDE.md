...
- **`src/plugin_api/`**: The plugin SDK — `MapApi`, `Window`, `RenderWindow`, `LayoutConfig`, `PolledFeed`, `InitialJump`, `Throttle`, `AsyncJob`, `NominatimClient`. Plugins import via `crate::plugin_api::prelude::*`.
...
- **`src/plugin/`**: Plugins (attribution, export, help, here, info, quake, scalebar, search, wiki) — composable UI panels with async work via `poll()`. Built-in chrome (info / scalebar / attribution) is structured as plugins too. Search is the unusual one: instead of being a `Component`, it's a `PaletteProvider` (in `src/plugin/search/mod.rs`) — its `register()` binds `/` to push the palette pre-loaded with the provider. Aircraft (live ADS-B) and ISS are Lua plugins and live under `src/lua/scripts/`; their existing `[aircraft]` / `[iss]` config gates route there now.
- **`src/lua/`**: Lua scripted plugins (mlua + Lua 5.4 vendored). `LuaComponent` (`component.rs`) implements `Component` by dispatching to a Lua module table. `host` (`host.rs`) is a per-component global exposing `host:fetch_url(url)`, `host:jump(lon, lat)`, `host:parse_json(s)`, `host:center()`. `MapApi` is bridged via a per-frame Lua table built inside `Lua::scope` (`map_api.rs`) and exposes `map:point(lon, lat, glyph, color)` and `map:label(lon, lat, text, color)`. **User plugins**: drop a `*.lua` file into `~/.config/ttymap/plugins/` and `register_user_plugins` picks it up at startup. Each script controls its own activation via the optional `module.enabled` field (default true) — the file *is* the config, no TOML side. `package.path` is extended at Lua-state construction so a plugin can `require "siblings"` from the same directory. Bundled `aircraft.lua` / `iss.lua` are loaded through `register_aircraft` / `register_iss` under the existing `[aircraft]` / `[iss]` TOML gates (because the user can't edit a script inside the binary); `register_hello` stays behind `[lua] enabled = true`. See `docs/lua-bridge-surface.md` for the bridge surface scope.
