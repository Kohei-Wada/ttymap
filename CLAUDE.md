...
- **`src/plugin_api/`**: Crate-internal primitives the Lua bridge re-uses — `MapApi` (drawing facade) and `PanelAnchor` / `LayoutConfig` (anchor vocabulary). The old plugin-author prelude (`PolledFeed`, `AsyncJob`, `Throttle`, `NominatimClient`, etc.) was retired together with the in-tree Rust plugins; equivalents live in Lua scripts now.
- **`src/widget/`**: Neutral widget-descriptor vocabulary (`Paragraph`, `List`, `Table`, …). The Lua bridge builds descriptors; `RenderWindow` does the `From<widget::X> for ratatui::X` conversion internally.
